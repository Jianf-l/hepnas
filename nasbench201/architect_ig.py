...
class Architect(object):
    ...
    def step(self, input_train, target_train, input_valid, target_valid, *args, **kwargs):
        if self.method == 'fo':
            shared = self._step_fo(input_train, target_train, input_valid, target_valid, kwargs['proj'])
        elif self.method == 'so':
    ...
    # first order
    def _step_fo(self, input_train, target_train, input_valid, target_valid, proj):
        loss = self.model._loss(input_valid, target_valid)
        if proj:
            # 添加KL

            # DARTS
            weights_normal, weights_reduce = self.model.get_theta()

            sorted_eid_alpha, op_id = torch.sort(weights_normal, dim=-1, descending=True)
            target_alpha = torch.tensor([1 if i == 0 else 0 for i in range(sorted_eid_alpha.shape[1])],
                                        device=sorted_eid_alpha.device).unsqueeze(0).repeat(sorted_eid_alpha.shape[0], 1)
            # 添加小量，使得alpha不为0
            sorted_eid_alpha_ = torch.where(sorted_eid_alpha == 0, torch.tensor(1e-4), sorted_eid_alpha)
            kl = F.kl_div(sorted_eid_alpha_.log().to(sorted_eid_alpha.device), target_alpha.float(), reduction='mean')
            loss = loss + kl

            sorted_eid_alpha, op_id = torch.sort(weights_reduce, dim=-1, descending=True)
            sorted_eid_alpha_ = torch.where(sorted_eid_alpha == 0, torch.tensor(1e-4), sorted_eid_alpha)
            kl = F.kl_div(sorted_eid_alpha_.log().to(sorted_eid_alpha.device), target_alpha.float(), reduction='mean')
            loss = loss + kl

            # # NAS-Bench-201
            # weights = self.model.get_theta()
            # sorted_eid_alpha, op_id = torch.sort(weights, dim=-1, descending=True)
            # target_alpha = torch.tensor([1 if i == 0 else 0 for i in range(sorted_eid_alpha.shape[1])],
            #                             device=sorted_eid_alpha.device).unsqueeze(0).repeat(sorted_eid_alpha.shape[0], 1)
            # sorted_eid_alpha_ = torch.where(sorted_eid_alpha == 0, torch.tensor(1e-4), sorted_eid_alpha)
            # kl = F.kl_div(sorted_eid_alpha_.log().to(sorted_eid_alpha.device), target_alpha.float(), reduction='mean')
    ...
    def _compute_unrolled_model(self, input, target, eta, network_optimizer):
        ...
        # remove unused ops in subnetworks
