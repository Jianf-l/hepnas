...
parser.add_argument('--edge_crit', type=str, default='random-fully',
                    choices=['predefine', 'random-perm', 'random-fully', 'grad'],
...
parser.add_argument('--supernet_train', type=str, default='darts', choices=['darts', 'pt', 'rsws'],
                    help='arch selection method')
parser.add_argument('--projection', type=str, default='darts', choices=['darts', 'pt', 'rsws'],
                    help='arch selection method')
# eval
...
def main():
    ...
    #     helper = lambda: TinyNetworkDartsProj(C=args.init_channels, N=5, max_nodes=4, num_classes=n_classes,
    #                                           criterion=criterion, search_space=search_space, args=args)
    elif args.method in ['ws', 'ws-so']:
        helper = lambda: TinyNetworkWS(C=args.init_channels, N=5, max_nodes=4, num_classes=n_classes,
                                       criterion=criterion, search_space=search_space, args=args,
                                       track_running_stats=args.bn_stats)
    ...
    for sid in args.sids.split(','):
        ...
        results = []  # (opids_str, genotype_str, valid_acc, true_valid_acc)
        ...
        for i in range(num_ops ** num_edges):
            ...
            # eval
            if not load_model.check_connect(theta_child):  # broken arch, 10%
                results.append([opids_str, 'invalid', torch.tensor([-1]).cuda(), -1])
            else:
                genotype, valid_acc = eval_supernet(valid_queue, load_model, criterion, theta_child)
                query_info = query(api, genotype, logging)
                results.append([opids_str, genotype.tostr(), valid_acc, query_info[data_flag]])
