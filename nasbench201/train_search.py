...
parser.add_argument('--edge_crit', type=str, default='hep',
                    choices=['predefine', 'random-perm', 'random-fully', 'grad', 'hep'],
...
parser.add_argument('--split_crit', type=str, default='grad', choices=['grad', 'fewshot'], help='grad = GM-NAS')
parser.add_argument('--split_num', type=int, default=2, choices=[2, 4], help='split into how many groups?')
parser.add_argument('--projection_warmup_epoch', type=int, default=0,
...
parser.add_argument('--projection', type=str, default='darts', choices=['darts', 'rsws', 'snas'],
...
parser.add_argument('--subnets_training_epoch', type=int, default=5,
                    help='how many epochs for training sub_supernets with knowledge distillation')
...
args.save = os.path.join('../experiments/', 'nasbench201', f'{args.group}/',
                         'search-{}-{}'.format(args.save, args.seed))
...
args.save += f'_[cutout-{args.cutout}]'
args.save += f'_[{args.kd_loss}]'
if args.restart != 'none':
    args.save += f'_[res-{args.restart}]'
args.split_ckpts = [int(x) for x in args.split_ckpts.split(',')]
if args.restart != 'none':
    args.warmup_epoch = args.split_ckpts[-1]
if args.warmup_epoch > 0:
    args.save += f'_[wm-{args.warmup_epoch}]'
if args.search_space == 'nas-bench-201':
    args.save += f'_[fullspace]'
if args.hard == 0:
    args.save += f'_[soft]'
if args.fs_opt:
    args.save += f'_[fsopt]'
if args.restart != 'none' and args.fix_sche:
...
def main():
    ...
    if args.method in ['darts', 'blank', 'darts-so']:
        helper = lambda: TinyNetworkDarts(C=args.init_channels, N=5, max_nodes=4, num_classes=n_classes,
                                          criterion=criterion, search_space=search_space, args=args)
    elif args.method in ['ws', 'ws-so']:
        helper = lambda: TinyNetworkWS(C=args.init_channels, N=5, max_nodes=4, num_classes=n_classes,
                                       criterion=criterion, search_space=search_space, args=args)
    elif args.method == 'ws-snas':
        helper = lambda: TinyNetworkSNASWS(C=args.init_channels, N=5, max_nodes=4, num_classes=n_classes,
                                           criterion=criterion, search_space=search_space, args=args)
    elif args.method == 'ws-rsws':
        helper = lambda: TinyNetworkRSWS(C=args.init_channels, N=5, max_nodes=4, num_classes=n_classes,
                                         criterion=criterion, search_space=search_space, args=args,
                                         track_running_stats=False)
    else:
        logging.info(f'ERROR: method {args.method} not supported')
        exit()
    ...
    offset = 0
    teacher = None
    subnets_train_epoch = args.subnets_training_epoch
    for level, end_epoch in enumerate(args.split_ckpts):
        ...
        for cur_model, cur_architect, cur_scheduler in supernets:
            ...
            encs_splitted = None
            if args.edge_crit == 'hep':
                assert len(args.split_ckpts) <= 3 and len(supernets) == 1
                ...
                for eid in range(split_node):
                    ...
                    encs_splitted = split_supernet(train_queue, valid_queue, cur_model,
                                                   cur_architect, criterion,
                                                   split_eid=split_eid,
                                                   split_crit=args.split_crit,
                                                   split_num=args.split_num)
                    all_encs_splitted.append(encs_splitted)
                ...
                for comb in combs:
                    ...
                    # the final encode
                    temp_encs_splitted = []
                    for encs_splitted, comb_index in zip(all_encs_splitted, comb):
                        temp_encs_splitted.append(encs_splitted[comb_index].bool())

                    encode = temp_encs_splitted[0]
                    for temp_ in temp_encs_splitted[1:]:
                        encode &= temp_
                    final_encs_splitted.append(encode.float())

                encs_splitted = final_encs_splitted
                ...
                if args.edge_crit == 'predefine':
                    ...
                    logging.info(f'Unrecognized edge criteria: {args.edge_crit}')
                    exit()

                encs_splitted = split_supernet(train_queue, valid_queue, cur_model, cur_architect, criterion,
                                               split_eid=split_eid, split_crit=args.split_crit,
                                               split_num=args.split_num)
        ...
        if args.edge_crit == 'hep':
            ...
            for st_epo in range(subnets_train_epoch):
                for i, (cur_model, cur_architect, cur_scheduler) in enumerate(supernets):
                    # train sub-supernets via knowledge distillation
                    ...
                    train_supernet(train_queue, valid_queue, cur_model, cur_architect, criterion, cur_scheduler,
                                   start_epoch + st_epo, start_epoch + st_epo + 1, train_transform, api, teacher,
                                   others)
            # subnets_train_epoch -= 1

            # 选择最优架构
            best_valid_acc = 0
            best_index = 0
            for i, (cur_model, _, _) in enumerate(supernets):
                logging.info('Evaluating sub_model id: %d', i)
                logging.info(cur_model.genotype())
                valid_acc, valid_obj = infer(valid_queue, cur_model, criterion, log=False, eval=('snas' in args.method))
                logging.info('valid_acc  %f', valid_acc)
                logging.info('valid_loss %f', valid_obj)
                if valid_acc > best_valid_acc:
                    best_valid_acc = valid_acc
                    best_index = i
            logging.info('select best sub_model id: %d', best_index)
            start_epoch = end_epoch
            supernets = [supernets[best_index]]
    ...
    for cur_model, cur_architect, cur_scheduler in supernets:
        cur_model.mode = args.projection  # set the supernet mode to rsws/darts
        train_supernet(train_queue, valid_queue, cur_model, cur_architect, criterion, cur_scheduler, start_epoch,
                       start_epoch + args.projection_warmup_epoch, train_transform, api, teacher=None, others=None, proj=True)
    ...
    best_valid_acc, best_alpha, best_enc = 0, None, None
...
def train_supernet(train_queue, valid_queue, model, architect, criterion, scheduler,
                   start_epoch, end_epoch, train_transform, api, teacher=None, others=None, proj=False):
    for epoch in range(start_epoch, end_epoch):
        ...
        # training
        train_acc, train_obj = train(train_queue, valid_queue, model, architect, model.optimizer, lr, epoch, teacher,
                                     others, proj)
        ...
        if not args.fast:
            ...
            cifar10_train, cifar10_test, cifar100_train, cifar100_valid, \
                cifar100_test, imagenet16_train, imagenet16_valid, imagenet16_test = query(api, model.genotype(),
                                                                                           logging)
...
def train(train_queue, valid_queue, model, architect, optimizer, lr, epoch, teacher=None, others=None, proj=False):
    ...
    for step in range(len(train_queue)):
        ...
        optimizer.zero_grad()
        architect.optimizer.zero_grad()
        logits, loss = model.step(input, target, args, shared=shared, teacher=teacher, others=others)
...
# util functions
def distill(result):
    cifar10_info = result.split('\n')[5].split('%')
    cifar10_train = cifar10_info[0][-5:]
    cifar10_test = cifar10_info[1][-5:]

    cifar100_info = re.search(
        r'cifar100\s+train : \[.*?top1 = (.*?)%], valid : \[.*?top1 = (.*?)%], test : \[.*?top1 = (.*?)%]',
        result).groups()
    ImageNet16_info = re.search(
        r'ImageNet16-120\s+train : \[.*?top1 = (.*?)%], valid : \[.*?top1 = (.*?)%], test : \[.*?top1 = (.*?)%]',
        result).groups()

    return float(cifar10_train), float(cifar10_test), float(cifar100_info[0]), float(cifar100_info[1]), \
        float(cifar100_info[2]), float(ImageNet16_info[0]), float(ImageNet16_info[1]), float(ImageNet16_info[2])


def query(api, genotype, logging):
    result = api.query_by_arch(genotype, '200')
