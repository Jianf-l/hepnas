...
from distill_loss import MultiTeacherLoss, KDLoss, DMLLoss
import sys
...
#         x_op = x[:, :dim_channel // self.k, :, :]
#         x_non_op = x[:, dim_channel // self.k:, :, :]
#         ret = sum(w * op(x_op) for w, op in zip(weights, self._ops) if w != 0)
#         if ret.shape[2] == x.shape[2]:
#             ans = torch.cat([ret, x_non_op], dim=1)
#         else:
#             ans = torch.cat([ret, self._mp(x_non_op)], dim=1)
...
class Network(nn.Module):
    def __init__(self, C, num_classes, layers, criterion, primitives, args,
        ...
        # Added Loss
        self._multi_teacher_loss = MultiTeacherLoss()
        self._dml_loss = DMLLoss()
    ...
    def _initialize_alphas_numpy(self, k, num_ops):
        ...
        # return Variable(1e-3*torch.randn(k, num_ops), requires_grad=True)
        if not self._args.fix_alpha_equal:
            return Variable(1e-3 * torch.randn(k, num_ops).cuda(), requires_grad=True)
        else:
            return Variable(1e-3 * torch.ones(k, num_ops).cuda(), requires_grad=True)
    ...
    def step(self, input, target, args, others=None, teacher=None, shared=None):
        assert shared is None, 'gradient sharing disabled'

        loss, logit_self = self._loss(input, target, return_logits=True)
        if others is not None and teacher is not None:
            with torch.no_grad():
                logit_teacher = teacher(input)
                logit_other = []
                for other in others:
                    logit_other.append(other(input))
            loss += self._multi_teacher_loss(logit_self, logit_teacher, logit_other)
            ...
            with torch.no_grad():
                ...
                for other in others:
                    logit_other.append(other(input))
            loss += self._dml_loss(logit_self, logit_other)

        elif teacher is not None:
            with torch.no_grad():
                logit_teacher = teacher(input)
            loss += self._kd_loss(logit_self, logit_teacher)
    ...
    def get_softmax(self) -> dict[str, torch.Tensor]:
    ...
    def new(self):  # note: this function should be made a virtural method in the future
        raise NotImplementedError  # avoid any unconscious usage
