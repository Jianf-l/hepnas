...
from model_search import Network, Cell


class NetworkWS(Network):
    def __init__(self, C, num_classes, layers, criterion, primitives, args,
                 steps=4, multiplier=4, stem_multiplier=3, drop_path_prob=0):
        super(NetworkWS, self).__init__(C, num_classes, layers, criterion, primitives, args, steps=steps,
                                        multiplier=multiplier, stem_multiplier=stem_multiplier,
    ...
    # none_id = 0 if discret_weights[3, none_id] != 1: return True if discret_weights[1, none_id] != 1 and
    # discret_weights[5, none_id] != 1: return True if discret_weights[0, none_id] != 1 and discret_weights[4,
    # none_id] != 1: return True if discret_weights[0, none_id] != 1 and discret_weights[2, none_id] != 1 and
    # discret_weights[5, none_id] != 1: return True

    def _sample_single_path_weights(self):  # randomly sample a single path
        while True:
            weights_normal = torch.zeros_like(self._arch_parameters[0])
            for eid in range(self.enc_normal.shape[0]):
                opids = torch.where(self.enc_normal[eid].cpu() == 1)[0]
                selected_opid = np.random.choice(opids)
                weights_normal[eid][selected_opid] = 1
            if self._check_connect(weights_normal):
                break
        while True:
            weights_reduce = torch.zeros_like(self._arch_parameters[1])
            for eid in range(self.enc_reduce.shape[0]):
                opids = torch.where(self.enc_reduce[eid].cpu() == 1)[0]
                selected_opid = np.random.choice(opids)
                weights_reduce[eid][selected_opid] = 1
            if self._check_connect(weights_reduce):
                break
        return weights_normal, weights_reduce

    def _get_softmax_weights(self):
        # weights_normal
        weights_normal = self._arch_parameters[0] * self.enc_normal
        for eid in range(self.enc_normal.shape[0]):
            weights_normal[eid][self.enc_normal[eid] == 1] = torch.softmax(
                weights_normal[eid][self.enc_normal[eid] == 1], dim=-1)
        # weights_reduce
        weights_reduce = self._arch_parameters[1] * self.enc_reduce
        for eid in range(self.enc_reduce.shape[0]):
            weights_reduce[eid][self.enc_reduce[eid] == 1] = torch.softmax(
                weights_reduce[eid][self.enc_reduce[eid] == 1], dim=-1)
        return weights_normal, weights_reduce
    ...
    def get_split_gradients(self, split_eid=0, split_normal=True):  # get all gradients except for edge [eid]
        ...
        param_grads = [p.grad for p in params if p.grad is not None]  # and p.grad.sum() != 0]
        return param_grads

    def get_theta(self):
        if self.mode == 'rsws':
            return self._sample_single_path_weights()
        elif self.mode == 'darts':
            return self._get_softmax_weights()
        else:
